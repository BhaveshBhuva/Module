 ...
 */
namespace Custom\ProductGallery\Block\Adminhtml\Product\Helper\Form\Gallery;
...
class Content extends \Magento\Backend\Block\Widget
{
     ...
     * @var string
     */
    protected $_template = 'Custom_ProductGallery::catalog/product/helper/gallery.phtml';
    ...
    protected function _prepareLayout()
    {
        ...
        $this->getUploader()->getConfig()->setUrl(
            $this->_urlBuilder->getUrl('productgallery/index/upload')
        )->setFileField(
            'image'
        )->setFilters(
            [
    ...
    {        
        ...
        $value=$this->_productGallery->create()->getCollection()->addFieldToFilter('product_id',$product->getId());
        ...
        if(!empty($value)){
            ...
            foreach ($value as $image) {
                $imagearray['url'] = $this->getBaseMediaUrl().$image->getValue();
                $imagearray['value_id']=$image->getValueId();
                $imagearray['file']=$image->getValue();
                $imagearray['media_type']='image';
                $imagearray['entity_id']=$image->getProductId();
                $imagearray['label']='';
                $imagearray['position']=$image->getPosition();
                $imagearray['disabled']='';
                $imagearray['label_default']='';
                $imagearray['disabled_default']='';
                $imagearray['position_default']='';
                try {                   
                    $fileHandler = $mediaDir->stat($mediaDir->getAbsolutePath().'productGallery'.$image->getValue());
                    $imagearray['size'] = $fileHandler['size'];
                } catch (FileSystemException $e) {
                    $imagearray['url'] = $this->getImageHelper()->getDefaultPlaceholderUrl('small_image');
                    $imagearray['size'] = 0;
